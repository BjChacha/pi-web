...
function sessiondTcpFallback() {
	...
	if (portSet !== urlSet) {
		stderr.write(
			`${ANSI.yellow}[dev] PI_WEB_SESSIOND_PORT and PI_WEB_SESSIOND_URL must be set together; filling in the missing one.${ANSI.reset}\n`,
		);
	}
	stdout.write(
		`${ANSI.gray}[dev] Windows detected: running sessiond over TCP 127.0.0.1:${SESSIOND_FALLBACK_PORT} (Unix domain sockets are not supported on native Windows).${ANSI.reset}\n`,
	);
	return {
		PI_WEB_SESSIOND_PORT:
			process.env["PI_WEB_SESSIOND_PORT"] || SESSIOND_FALLBACK_PORT,
		PI_WEB_SESSIOND_URL:
			process.env["PI_WEB_SESSIOND_URL"] || SESSIOND_FALLBACK_URL,
	};
}
...
const tagFor = (task) =>
	`${ANSI[task.color]}[${task.label.padEnd(labelWidth)}]${ANSI.reset}`;

stdout.write(
	`${ANSI.gray}[dev] starting ${String(tasks.length)} task(s): ${tasks.map((task) => task.label).join(", ")}${ANSI.reset}\n`,
);
...
for (const task of tasks) {
	...
	child.on("error", (error) => {
		stderr.write(
			`${tagFor(task)} ${ANSI.red}failed to start: ${error.message}${ANSI.reset}\n`,
		);
		markExited(task, 1);
	});
}
