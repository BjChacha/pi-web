...
import {
	...
	type SessionHeaderSummary,
} from "./sessionFileHeader.js";
import {
	countOutOfListingChildren,
	locateOutOfListingParents,
	type SessionHeaderReader,
} from "./parentSessionLocator.js";
import {
	siblingWorkspaceCwds,
	type ProjectWorkspaceCwds,
} from "../workspaces/projectWorkspaceCwds.js";
...
import {
	ExtensionDialogWaiters,
	effectiveExtensionDialogTimeoutMs,
	extensionDialogCancelValue,
} from "./extensionDialogWaiters.js";
 ...
 */
const STARTUP_CONCURRENT_CATALOG_REFRESH =
	"provider model lists are refreshing";
...
function unknownSpawnModelError(modelSpec: string): Error {
	return new Error(
		`Unknown model "${modelSpec}". Pass an exact "provider/model-id".`,
	);
}
...
function lookupMatchesStartupSession(
	ref: PiSessionLookup,
	session: PiAgentSession,
): boolean {
	return (
		!isPiSessionRef(ref) ||
		cwdPathsEqual(session.sessionManager.getCwd(), ref.cwd)
	);
}
...
export interface PiAgentSession {
	...
	/** Names of the tools currently active on the agent. Plan-mode extensions remove write tools (edit/write) to enter plan mode. */
...
export class PiSessionService implements SessionRouteService {
	...
	constructor(
		...
		this.pendingExtensionDialogStore =
			deps.pendingExtensionDialogStore ?? new PendingExtensionDialogStore();
		this.extensionDialogsTimeoutMs =
			deps.extensionDialogsTimeoutMs ?? DEFAULT_EXTENSION_DIALOGS_TIMEOUT_MS;
		...
		this.createRuntime =
			...
			createDefaultRuntimeFactory(
				...
				deps.askUserEnabled === true
					? { open: (input) => this.openAsk(input) }
					: undefined,
			);
	...
	async list(cwd: string): Promise<ClientSession[]> {
		...
		const archivedSessions = archivedForCwd
			...
			.filter(isDefined);
		return await this.withRelatedSessionsElsewhere(
			[...unarchivedSessions, ...archivedSessions],
			cwd,
		);
	}
	 ...
	 */
	private async withRelatedSessionsElsewhere(
		...
		return sessions.map((session) => {
			const parent =
				session.parentSessionPath === undefined
					? undefined
					: parentLocations.get(session.parentSessionPath);
			const childrenElsewhere = childCounts.get(session.path);
			if (parent === undefined && childrenElsewhere === undefined)
				return session;
			...
			if (childrenElsewhere !== undefined)
				annotated.childSessionsElsewhere = childrenElsewhere;
			return annotated;
		});
	}
	 ...
	 */
	private async countChildrenInSiblingWorkspaces(
		sessions: readonly ClientSession[],
		cwd: string,
	): Promise<Map<string, number>> {
		if (this.projectWorkspaces === undefined || sessions.length === 0)
			return new Map();
		try {
			const siblingCwds = await siblingWorkspaceCwds(
				this.projectWorkspaces,
				cwd,
			);
			if (siblingCwds.length === 0) return new Map();
			const listings = await Promise.all(
				siblingCwds.map(async (siblingCwd): Promise<string[]> => {
					const entries = await this.sessionManager.list(siblingCwd);
					return entries.flatMap((entry) =>
						entry.parentSessionPath === undefined
							? []
	 ...
	 */
	private readonly readCachedSessionHeader: SessionHeaderReader = async (
		sessionFile,
	) => {
	...
	async spawnSession(
		...
		const model =
			input.modelSpec === undefined
				? input.model
				: await this.resolveSpawnModel(
						input.spawningSessionId,
						input.modelSpec,
					);
	 ...
	 */
	private async sessionModelCandidates(
		session: PiAgentSession,
	...
	async submitAsk(
		...
		return this.closeAsk(
			session,
			this.pendingAskStore.submit(session.sessionId, askId, submission),
		);
	}
	...
	async cancelAsk(
		...
		return this.closeAsk(
			session,
			this.pendingAskStore.cancel(session.sessionId, askId),
		);
	}
	...
	private async closeAsk(
		session: PiAgentSession,
		result: PendingAskCloseResult,
	): Promise<AskUserCloseResponse> {
		if (result.status === "stale")
			return {
				result: "stale",
				sessionStatus: this.statusFromSession(session),
			};
		const { outcome } = result;
		this.publishAskClosed(session.sessionId, outcome);
		await this.runSessionEntryMutation(
			session,
			"deliver answers to your questions",
			() =>
	 ...
	 */
	private async voidOpenAskForUserMessage(
		session: PiAgentSession,
	): Promise<void> {
		const outcome = this.pendingAskStore.cancelOpen(session.sessionId);
		if (outcome === undefined) return;
		this.publishAskClosed(session.sessionId, outcome);
		await this.runSessionEntryMutation(session, "void the open questions", () =>
			session.sendCustomMessage(
				{
					customType: ASK_USER_ANSWERS_CUSTOM_TYPE,
					content: renderAskUserAnswersText(outcome),
	...
	): Promise<ExtensionDialogCloseResponse> {
		await this.assertWritable(ref);
		const session = await this.sessionForStatusOrDialogClose(ref);
		const result = this.pendingExtensionDialogStore.answer(
			session.sessionId,
			dialogId,
			value,
		);
	...
	async cancelDialog(
		...
		await this.assertWritable(ref);
		const session = await this.sessionForStatusOrDialogClose(ref);
		const result = this.pendingExtensionDialogStore.cancel(
			session.sessionId,
			dialogId,
			"cancelled",
		);
	...
	private async openExtensionDialog(
		...
		const timeoutMs = effectiveExtensionDialogTimeoutMs(
			opts?.timeout,
			this.extensionDialogsTimeoutMs,
		);
		...
		return this.dialogWaiters.park(dialog, {
			...
			onTrigger: (reason) => {
				if (
					this.closeExtensionDialogFromTrigger(
						session.sessionId,
						dialog.dialogId,
						reason,
					)
	 ...
	 */
	private closeExtensionDialogFromTrigger(
		sessionId: string,
		dialogId: string,
		reason: ExtensionDialogCancelReason,
	): boolean {
		const result = this.pendingExtensionDialogStore.cancel(
			sessionId,
			dialogId,
	...
	private abortRunScopedExtensionDialogs(sessionId: string): void {
		let closedAny = false;
		for (const dialog of this.pendingExtensionDialogStore.pendingDialogs(
			sessionId,
		)) {
			if (dialog.runScoped)
				closedAny =
	...
	async status(ref: PiSessionLookup): Promise<ClientSessionStatus> {
		return this.statusFromSession(
			await this.sessionForStatusOrDialogClose(ref),
		);
	}
	...
	async streamSnapshot(ref: PiSessionLookup): Promise<SessionStreamSnapshot> {
		...
		const partial =
			streamingMessage === undefined || streamingMessage === null
				? null
				: annotateAssistantThinkingLevel(
	 ...
	 */
	private async sessionForStatusOrDialogClose(
		ref: PiSessionLookup,
	): Promise<PiAgentSession> {
		const reachable =
			this.activeForLookup(ref)?.runtime.session ??
			this.startupSessionForLookup(ref);
	...
	private async create(
		...
		try {
			return await this.createSessionRuntime(
				sessionManager,
				cwd,
				options,
				startup,
	...
	private bindRuntime(
		...
		active.unsubscribe = session.subscribe((event) => {
			...
			this.events.publish(
				session.sessionId,
				toClientEvent(event, session.thinkingLevel),
			);
	...
	private startupProgress(
		...
		return {
			report: (phase) => {
				this.publishStartupProgress(
					sessionId,
					startupToken,
					label,
					"active",
					this.startupDetail(phase),
				);
			},
			end: () => {
	...
	private publishStartupProgress(
		...
		const activity =
			detail === undefined
				? { sessionId, phase, label, at, startup: true }
				: { sessionId, phase, label, detail, at, startup: true };
	...
	private statusFromSession(session: PiAgentSession): ClientSessionStatus {
		...
		const pendingDialogs = this.pendingExtensionDialogStore.pendingDialogs(
			session.sessionId,
		);
		const tokenRate = this.tokenRateFor(session);
		const planModeActive = planModeActiveForSession(session);
		return {
			sessionId: session.sessionId,
			persisted: sessionFileExists(session.sessionFile),
			...
			...(planModeActive === undefined ? {} : { planModeActive }),
	...
	private updateTokenRate(session: PiAgentSession, event: unknown): void {
		...
		if (eventType === "message_update") {
			...
			tracker.streamedChars +=
				getString(assistantMessageEvent, "delta")?.length ?? 0;
			...
			if (output !== undefined && output > 0)
				tracker.streamedOutputTokens = output;
			return;
		}
		if (eventType === "message_end") {
			const message = getProperty(event, "message");
			...
			if (startedAtMs !== undefined) {
				...
				const tokens =
					output !== undefined && output > 0
						? output
						: Math.round(tracker.streamedChars / ESTIMATED_CHARS_PER_TOKEN);
				const elapsedMs = Date.now() - startedAtMs;
				if (tokens > 0 && elapsedMs >= MIN_TOKEN_RATE_ELAPSED_MS)
					tracker.lastRate = tokens / (elapsedMs / 1000);
			}
 ...
 */
function annotateAssistantThinkingLevel(
	message: unknown,
...
function toClientEvent(event: unknown, thinkingLevel?: string): SessionUiEvent {
	...
	if (eventType === "message_end") {
		...
		return {
			type: "message.end",
			message: annotateAssistantThinkingLevel(message, thinkingLevel),
		};
	}
...
/**
 * Determine plan-mode state for a session's status. Plan-mode extensions
 * (official example, pi-plan-mode, ...) persist the toggle immediately via
 * appendEntry("plan-mode", { active | enabled }), while the tool-set change
 * (setActiveTools removing edit/write) only takes effect on the next
 * before_agent_start. So prefer the persisted entry — it reflects the live
 * toggle without the tool-set lag — and fall back to inferring from the active
 * tool set for extensions that toggle tools without persisting.
 */
function planModeActiveForSession(session: PiAgentSession): boolean | undefined {
	...
	if (entries !== undefined) {
		for (let i = entries.length - 1; i >= 0; i--) {
			...
			const data = entry["data"];
			if (isRecord(data)) {
				const active = data["active"] ?? data["enabled"];
