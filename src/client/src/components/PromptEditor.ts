...
import { loadPlanModeActive, savePlanModeActive } from "../planModeState";
import {
...
export function planModeCommandName(
	commands: readonly SlashCommand[],
): string | undefined {
	return commands.find(
		(command) =>
			command.source === "extension" &&
			PLAN_MODE_COMMAND_NAMES.has(command.name),
	)?.name;
}
...
export class PromptEditor extends LitElement {
	...
	/** Best-effort on/off view of plan mode, persisted per session. See {@link planModeState}. */
	@state() private localPlanModeActive = false;
	...
	protected override willUpdate(changed: PropertyValues<this>) {
		...
		this.localPlanModeActive =
			this.sessionId !== undefined && this.sessionId !== ""
				? loadPlanModeActive(this.machineId, this.sessionId)
				: false;
	...
	private renderCompactStatus() {
		...
		const provider =
			status.model?.provider !== undefined && status.model.provider !== ""
				...
				: "";
		// Prefer the daemon's authoritative plan-mode state; fall back to the local
	...
	private readonly handleTogglePlan = (): void => {
		if (this.planCommandName === undefined) return;
		const next = !(this.status?.planModeActive ?? this.localPlanModeActive);
		this.localPlanModeActive = next;
		if (this.sessionId !== undefined && this.sessionId !== "")
			savePlanModeActive(this.machineId, this.sessionId, next);
		this.onTogglePlan?.(this.planCommandName);
	};
	...
	private async refreshPlanModeCommand(): Promise<void> {
		...
		const commands = await api
			.commands({ id: sessionId, cwd }, machineId)
      ...
      <div class="attachments" aria-label="Pending attachments">
        ${this.attachments.map(
          ...
          <div class=${`attachment-chip ${isInlinePromptAttachment(attachment) ? "attachment-chip-image" : "attachment-chip-file"}`} title=${attachment.name}>
            ...
            <button type="button" class="attachment-remove" title="Remove attachment" aria-label=${`Remove ${attachment.name}`} @click=${() => {
		...
		this.editor = new EditorView({
			...
			state: EditorState.create({
				...
				extensions: [
					...
					placeholder(
						"Message pi... Use / for commands, @ for tracked files, @ space for all files, # for models",
					),
	...
	private async refreshCompletions() {
		...
		if (
			...
			const models = await api
				.models({ id: this.sessionId, cwd: this.cwd }, this.machineId)
				.then((response) => response.models)
				.catch(emptySessionModels);
			if (version !== this.requestVersion) return;
			this.completions = modelCompletionChoices(models, trigger.query).map(
				(choice) => ({
					kind: "model",
					replaceFrom: trigger.from,
					replaceTo: trigger.to,
...
function sessionStatusRenderEqual(
	...
	return (
		...
		a.planModeActive === b.planModeActive
	);
}
