...
import {
	...
	parseSessionStartupProgressEvent,
	parseSessionStatus,
...
describe("API parsers", () => {
	...
	it("carries the startup marker so an opening session is not mistaken for a working one", () => {
		...
		expect(
			parseSessionStartupProgressEvent({ type: "session.startup", activity }),
		).toEqual({ type: "session.startup", activity });
		...
		expect(() =>
			parseSessionStartupProgressEvent({
				type: "session.startup",
				activity: { ...activity, startup: "yes" },
			}),
		).toThrow("Expected optional boolean field: startup");
	});
	...
	it("rejects session startup progress that cannot be routed or rendered honestly", () => {
		...
		expect(() =>
			parseSessionStartupProgressEvent({ type: "activity.update", activity }),
		).toThrow("Invalid session startup event type");
		expect(() =>
			parseSessionStartupProgressEvent({ type: "session.startup" }),
		).toThrow("Expected object response");
		expect(() =>
			parseSessionStartupProgressEvent({
				type: "session.startup",
				startupToken: 7,
		...
		).toThrow("Expected optional string field: startupToken");
		...
		).toThrow("Expected non-empty string field: startupToken");
		expect(() =>
			parseSessionStartupProgressEvent({
				type: "session.startup",
				activity: { ...activity, phase: "waiting" },
			}),
		).toThrow("Expected session activity phase field: phase");
		expect(() =>
			parseSessionStartupProgressEvent({
				type: "session.startup",
				activity: { ...activity, label: 7 },
			}),
		).toThrow("Expected string field: label");
		expect(() =>
			parseSessionStartupProgressEvent({
				type: "session.startup",
				activity: { ...activity, label: "" },
			}),
		).toThrow("Expected non-empty string field: label");
		expect(() =>
			parseSessionStartupProgressEvent({
				type: "session.startup",
				activity: { ...activity, detail: 7 },
			}),
		).toThrow("Expected optional string field: detail");
		expect(() =>
			parseSessionStartupProgressEvent({
				type: "session.startup",
				activity: { ...activity, sessionId: "" },
			}),
		).toThrow("Expected non-empty string field: sessionId");
	});
	...
	it("parses the optional plan-mode flag when present and omits it otherwise", () => {
		expect(
			parseSessionStatus({
				sessionId: "s1",
				isStreaming: false,
			...
			questions: [
				{
					...
					detail: "Pick the primary store",
					options: [
						{ value: "pg", label: "Postgres", detail: "Relational" },
				...
				},
				{
					...
					options: [{ value: "metrics", label: "Metrics" }],
					allowOther: true,
					multiple: true,
				},
			],
	...
	it("validates an ask before rendering it", () => {
		...
		expect(() =>
			parseSessionStatus({
				...statusWire(),
				pendingAsk: { ...ask, questions: [] },
			}),
		).toThrow("Pending ask has no questions");
		expect(() =>
			parseSessionStatus({
				...statusWire(),
				pendingAsk: { ...ask, questions: [first, first] },
			}),
		).toThrow("Duplicate ask question id");
		expect(() =>
			parseSessionStatus({
				...statusWire(),
				pendingAsk: { ...ask, askId: "" },
			}),
		).toThrow("Expected non-empty string field: askId");
		expect(
			parseSessionStatus({
				...statusWire(),
				pendingAsk: {
		...
		expect(() =>
			parseSessionStatus({
				...
				pendingAsk: {
					...ask,
					questions: [
						{ id: "q1", question: "Anything?", options: [], allowOther: "yes" },
					],
				},
			}),
		).toThrow("Expected optional boolean field: allowOther");
		expect(() =>
			parseSessionStatus({
				...statusWire(),
				pendingAsk: {
		...
		).toThrow("Duplicate ask option value");
		expect(() =>
			parseSessionStatus({
				...statusWire(),
				pendingAsk: {
					...
					questions: [
						{
							id: "q1",
							question: "x".repeat(ASK_USER_TEXT_MAX_LENGTH + 1),
							options: [{ value: "a", label: "A" }],
						},
					],
				},
			}),
		).toThrow("String field exceeds limit: question");
	});
	...
	it("parses a closed ask response carrying the outcome and recomputed status", () => {
		...
		expect(response.outcome?.questions[0]).toMatchObject({
			answered: true,
			values: ["pg"],
		});
	...
	it("parses a stale close as an ordinary race with no outcome", () => {
		...
		expect(response).toEqual({
			result: "stale",
			sessionStatus: parseSessionStatus(statusWire()),
		});
	});
	...
	it("rejects close responses whose outcome contradicts itself", () => {
		...
		).toThrow("Ask close response outcome mismatch");
		expect(() =>
			parseAskUserCloseResponse({
				result: "stale",
				outcome,
		...
		expect(() =>
			parseAskUserCloseResponse({
				result: "closed",
				outcome: { ...outcome, answeredCount: 2 },
				sessionStatus: statusWire(),
			}),
		).toThrow("Ask outcome answered count mismatch");
		expect(() =>
			parseAskUserCloseResponse({
				result: "closed",
				outcome: { ...outcome, unansweredIds: [] },
				sessionStatus: statusWire(),
			}),
		).toThrow("Ask outcome unanswered ids mismatch");
		expect(() =>
			parseAskUserCloseResponse({
				result: "closed",
				outcome: { ...outcome, reason: "ignored" },
				sessionStatus: statusWire(),
			}),
		).toThrow("Invalid ask close reason");
		expect(() =>
			parseAskUserCloseResponse({
				result: "closed",
				outcome: {
					...outcome,
					questions: [
						{ ...askAnsweredRecordWire(), answered: false },
						askUnansweredRecordWire(),
					],
				},
				sessionStatus: statusWire(),
			}),
		...
		expect(() =>
			parseAskUserCloseResponse({
				...
				outcome: {
					...outcome,
					questions: [
						{ ...askAnsweredRecordWire(), values: ["mysql"] },
						askUnansweredRecordWire(),
					],
				},
				sessionStatus: statusWire(),
	...
	it("validates an extension dialog before rendering it", () => {
		...
		expect(() =>
			parseSessionStatus({
				...statusWire(),
				pendingDialogs: [{ ...dialog, kind: "modal" }],
			}),
		).toThrow("Invalid extension dialog kind");
		expect(() =>
			parseSessionStatus({
				...statusWire(),
				pendingDialogs: [{ ...dialog, title: "" }],
			}),
		).toThrow("Expected non-empty string field: title");
		expect(() =>
			parseSessionStatus({
				...statusWire(),
				pendingDialogs: [
					{
						...dialog,
						title: "x".repeat(EXTENSION_DIALOG_TEXT_MAX_LENGTH + 1),
					},
				],
			}),
		).toThrow("String field exceeds limit: title");
		expect(() =>
			parseSessionStatus({
				...statusWire(),
				pendingDialogs: [{ ...dialog, runScoped: "yes" }],
			}),
		).toThrow("Expected boolean field: runScoped");
		expect(() =>
			parseSessionStatus({
				...statusWire(),
				pendingDialogs: [{ ...dialog, timeoutAt: "" }],
			}),
		).toThrow("Expected non-empty string field: timeoutAt");
		expect(() =>
			parseSessionStatus({
				...statusWire(),
				pendingDialogs: [{ ...selectDialogWire(), options: [] }],
			}),
		).toThrow("Select dialog has no options");
		expect(() =>
			parseSessionStatus({
				...statusWire(),
				pendingDialogs: [{ ...selectDialogWire(), options: ["a", "a"] }],
			}),
		).toThrow("Duplicate dialog option");
		expect(() =>
			parseSessionStatus({
				...statusWire(),
				pendingDialogs: [
					dialog,
					{ ...inputDialogWire(), dialogId: "dialog-1" },
				],
			}),
		).toThrow("Duplicate dialog id");
	});
	...
	it("rejects dialog close responses whose outcome contradicts itself", () => {
		...
		).toThrow("Dialog close response outcome mismatch");
		expect(() =>
			parseExtensionDialogCloseResponse({
				result: "stale",
				outcome,
		...
		expect(() =>
			parseExtensionDialogCloseResponse({
				result: "closed",
				outcome: { ...outcome, reason: "timeout" },
				sessionStatus: statusWire(),
			}),
		).toThrow("Dialog outcome answer mismatch");
		expect(() =>
			parseExtensionDialogCloseResponse({
				result: "closed",
				outcome: { ...outcome, answer: 1 },
				sessionStatus: statusWire(),
			}),
		).toThrow("Invalid extension dialog answer");
		expect(() =>
			parseExtensionDialogCloseResponse({
				result: "closed",
				outcome: { ...outcome, reason: "ignored" },
		...
		).toThrow("Invalid extension dialog close reason");
	});
});
